...
import type { ChatMessage } from "@orahai/types";
...
export function ChatPanel({ projectId, activeFilePath, activeFileContent }: ChatPanelProps) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("orahai_token") : null;
      const res = await fetch(`${API_BASE}/api/ai/chat/${projectId}`, {
        method: "POST",
        headers: {
