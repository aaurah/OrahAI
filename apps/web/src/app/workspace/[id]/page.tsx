...
import type { ProjectFile, ApiResponse, Run } from "@orahai/types";

export default function WorkspacePage() {
  const { id } = useParams<{ id: string }>();
  const { project, isLoading } = useProject(id);
  const { runs, mutate: mutateRuns } = useRuns(id);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      // Trigger immediate SWR revalidation; useRuns will auto-poll while active
      await mutateRuns();
    } catch (err: unknown) {
