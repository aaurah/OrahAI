...
import type { ApiResponse, Run } from "@orahai/types";
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const term = new XTerm({
        theme: {
          background: "#0d0d0d",  foreground: "#d4d4d4",  cursor: "#ffffff",
      ...
      term.onKey(({ key, domEvent }) => {
        if (domEvent.keyCode === 13) {
          ...
          if (cmd) {
            // POST to /api/runs/:projectId with the typed command
            api.post<ApiResponse<Run>>(`/api/runs/${projectId}`, { command: cmd })
              .then((res) => {
                const run = res.data;
                if (run.output) {
                  term.write(run.output.replace(/\n/g, "\r\n"));
                }
      ...
      // Real-time output via WebSocket (sandbox streams here)
      if (socket) {
