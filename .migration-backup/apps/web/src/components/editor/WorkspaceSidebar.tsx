...
import type { FileNode, ProjectFile } from "@orahai/types";
...
export function WorkspaceSidebar({ projectId, activeFilePath, onFileSelect }: Props) {
  ...
  const [creating, setCreating] = useState<string | null>(null); // parent dir path
  ...
  const commitCreate = async () => {
    ...
    try {
      ...
      await mutate();
      // Select the new file
      const created = { id: "", projectId, path, name: newName.trim(), content: "", mimeType: "text/plain", isDir: false, size: 0, createdAt: "", updatedAt: "" };
