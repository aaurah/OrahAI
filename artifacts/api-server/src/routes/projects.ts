  ...
  const [p] = await db.select().from(projects)
    .where(and(
      ...
      or(
        ...
        sql`${projects.workspaceId} IN (${memberSubquery})`,
      ),
    )).limit(1);
...
router.get("/", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const conditions = [
      ...
      ...(search ? [ilike(projects.name, `%${search}%`)] : []),
    ].filter(Boolean);
    ...
    }).from(projects).where(and(...(conditions as [ReturnType<typeof and>])));
...
router.patch("/:id", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const parsed = schema.safeParse(req.body);
    if (!parsed.success) return next(createError("Validation error", 400, parsed.error.errors));
    const [updated] = await db.update(projects).set({ ...parsed.data, updatedAt: new Date() }).where(eq(projects.id, req.params.id)).returning();
...
router.delete("/:id", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const [p] = await db.select().from(projects).where(and(eq(projects.id, req.params.id), eq(projects.ownerId, req.user!.id), isNull(projects.deletedAt))).limit(1);
    if (!p) return next(createError("Project not found", 404));
