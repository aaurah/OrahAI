...
router.get("/", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const rows = await db
      ...
      .where(and(eq(memberships.userId, req.user!.id), eq(workspaces.deletedAt, null as unknown as Date)));
    res.json({ data: rows });
  } catch (err) { next(err); }
});
...
router.get("/:id", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const rows = await db
      ...
      .from(memberships)
      .innerJoin(workspaces, eq(memberships.workspaceId, workspaces.id))
      .where(and(eq(memberships.workspaceId, req.params.id), eq(memberships.userId, req.user!.id)))
      .limit(1);
...
router.patch("/:id", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const parsed = schema.safeParse(req.body);
    if (!parsed.success) return next(createError("Validation error", 400, parsed.error.errors));
    const [ws] = await db.update(workspaces).set({ ...parsed.data, updatedAt: new Date() }).where(eq(workspaces.id, req.params.id)).returning();
    res.json({ data: ws });
  } catch (err) { next(err); }
});
...
router.delete("/:id", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await db.update(workspaces).set({ deletedAt: new Date() }).where(eq(workspaces.id, req.params.id));
...
router.post("/:id/members", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const [existing] = await db.select().from(memberships)
      .where(and(eq(memberships.userId, user.id), eq(memberships.workspaceId, req.params.id))).limit(1);
    if (existing) return next(createError("User is already a member", 409));
    const [member] = await db.insert(memberships).values({ id: cuid(), userId: user.id, workspaceId: req.params.id, role: parsed.data.role }).returning();
...
router.delete("/:id/members/:userId", requireAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await db.delete(memberships).where(and(eq(memberships.userId, req.params.userId), eq(memberships.workspaceId, req.params.id)));
