...
function buildSystemPrompt(
  ...
  const lines: string[] = [
    ...
    ``,
    `⚠️  ORAHAI PREVIEW — TWO MODES:`,
    ``,
    `   1. FRONTEND TAB (static):  Serves public/index.html, public/app.js, public/style.css directly.`,
    `      No server executes here. Any fetch("/api/custom-route") will hit OrahAI's own API and return 404.`,
    `      Use public APIs (CoinGecko, Open-Meteo, etc.) and localStorage for purely static projects.`,
    ``,
    `   2. LIVE TAB (running process):  The user clicks the ▶ Run button → OrahAI spawns the project's`,
    `      start command (e.g. "npm run dev", "python main.py", "flask run") as a real OS process.`,
    `      When the server starts listening on a port, the Live tab appears and proxies all traffic to it.`,
    `      THIS IS THE RIGHT MODE for Express, Flask, FastAPI, Next.js, Go HTTP, Rust Actix, etc.`,
    ``,
    `✅ For projects with a backend server: write the full server code + tell the user to click ▶ Run.`,
    `✅ For pure frontend / static projects: use public/ files + public APIs + localStorage.`,
    `✅ When describing how to access a running server: say "click ▶ Run, then open the Live tab".`,
    `✅ For scripts / CLI tools / libraries (no UI): write the code + show the run command in a code block.`,
    ``,
    `════════════════════════════════════════════`,
